...
void cameraCapturar(HWND hwnd) {
    ...
    DWORD flags = 0;
    HRESULT hr = reader->lpVtbl->ReadSample(reader, MF_SOURCE_READER_FIRST_VIDEO_STREAM, 0, NULL, &flags, NULL, &sample);
    ...
    if (FAILED(hr)) {
        MessageBoxA(hwnd, "Nao foi possivel acessar os dados do frame.","Erro de captura", MB_ICONERROR);
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (tamanhoAtual >= 640U * 480U * 4U) {
            ...
            if (!tamanhoReportado) {
                char mensagem[160];
                sprintf(mensagem, "Frame recebido com tamanho inesperado: %lu bytes.",(unsigned long)tamanhoAtual);
                MessageBoxA(hwnd, mensagem, "Erro de captura", MB_ICONERROR);
                tamanhoReportado = 1;
            }
        }
...
HRESULT cameraInicializar(int indiceDispositivo) {
    ...
    hr = atributos->lpVtbl->SetGUID(atributos, &MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE,&MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_GUID);
    ...
    hr = dispositivos[indiceDispositivo]->lpVtbl->ActivateObject(dispositivos[indiceDispositivo], &IID_IMFMediaSource,(void **)&fonte);
    ...
    hr = tipo->lpVtbl->SetUINT64(tipo, &MF_MT_FRAME_SIZE,((UINT64)640 << 32) | (UINT64)480);
