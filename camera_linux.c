...
int cameraInicializarLinux(const char *dispositivo, int largura, int altura) {
    ...
    if (formato.fmt.pix.pixelformat != V4L2_PIX_FMT_RGB24 && formato.fmt.pix.pixelformat != V4L2_PIX_FMT_YUYV && formato.fmt.pix.pixelformat != V4L2_PIX_FMT_MJPEG) {
    ...
    for (unsigned int i = 0; i < quantidadeBuffers; i++) {
        ...
        buffers[i].dados = mmap(NULL, buffer.length, PROT_READ | PROT_WRITE, MAP_SHARED, camera, buffer.m.offset);
...
int cameraCapturarLinux(void) {
    ...
    if (formatoCamera == V4L2_PIX_FMT_MJPEG) {
        ...
        size_t tamanhoDecodificado = (size_t)decodificador.output_width *decodificador.output_height * decodificador.output_components;
        ...
        while (decodificador.output_scanline < decodificador.output_height) {
            linha = frameDecodificado + decodificador.output_scanline * decodificador.output_width * decodificador.output_components;
